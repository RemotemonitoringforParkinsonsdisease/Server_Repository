...
public class PatientJDBC implements PatientManager {
    ...
    // Metodo para obtener un paciente por su patient_id
    public Patient getPatientByPatientId(Integer patientId) {
        ...
        try (PreparedStatement stmt = manager.getConnection().prepareStatement(sql)) {
            ...
            if (rs.next()) {
                ...
                // Si tienes un metodo como getReportsByPatientId, puedes llamarlo aquí y agregar los reportes al paciente:
    ...
    // Metodo para obtener un paciente por su email (suponiendo que hay un campo email en la base de datos)
